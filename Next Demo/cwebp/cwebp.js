...
function getFileList(src, dst) {
  ...
  paths.forEach((pathname) => {
    ...
    if (st.isFile()) {
      ...
      if (
        !fs.existsSync(_dst) &&
        !handleredFolderList.includes(path.resolve(CWD, _dst))
      ) {
        fs.mkdirSync(_dst);
      }
      // 递归调用
...
function handlerCwebp(list, index = 0) {
  ...
  const statsOrigin = fs.statSync(list[0].src);

  const statsWebp = fs.statSync(list[0].dst);

  //   如果原图更小，那么直接使用原图
  if (statsOrigin.size < statsWebp.size) {
    fs.unlinkSync(list[0].src);
    fs.unlinkSync(list[0].dst);
    console.log(
      "原图更大，被删除,原图小--》",
